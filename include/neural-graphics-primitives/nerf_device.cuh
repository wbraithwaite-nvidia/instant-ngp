...
inline constexpr __device__ uint32_t NERF_CASCADES()
{
    return 8;
}

inline constexpr __device__ float SQRT3()
{
...
inline constexpr __device__ float NERF_MIN_OPTICAL_THICKNESS()
{
    return 0.01f;
}
...
struct NerfPayload
{
    vec3 origin;      // world-space ray origin.
    vec3 dir;         // normalized world-space ray direction.
    float t;          // ray parameter.
    float max_weight; // max accumulated weight.
    uint32_t idx;     // offset of pixel in framebuffer.
    uint16_t n_steps; // number of steps this ray has taken.
    bool alive;       // has this ray terminated?
};
...
inline NGP_HOST_DEVICE bool density_grid_occupied_at(const vec3& pos,
...
inline NGP_HOST_DEVICE float distance_to_next_voxel(const vec3& pos, const vec3& dir, const vec3& idir, float res)
{ // dda like step
    vec3 p   = res * (pos - 0.5f);
...
inline NGP_HOST_DEVICE float advance_to_next_voxel(float t,
                                                   float cone_angle,
                                                   const vec3& pos,
                                                   const vec3& dir,
                                                   const vec3& idir,
...
NGP_HOST_DEVICE float if_unoccupied_advance_to_next_occupied_voxel(float t,
                                                                   float cone_angle,
                                                                   const Ray& ray,
                                                                   const vec3& idir,
                                                                   ...
                                                                   mat3 aabb_to_local = mat3::identity())
{
    while (true)
    {
        ...
        uint32_t mip =
            clamp(MIP_FROM_DT ? mip_from_dt(calc_dt(t, cone_angle), pos) : mip_from_pos(pos), min_mip, max_mip);
...
inline NGP_HOST_DEVICE vec2 sample_cdf_2d(vec2 sample,
                                          uint32_t img,
                                          const ivec2& res,
...
inline NGP_HOST_DEVICE float pdf_2d(vec2 sample,
                                    uint32_t img,
                                    const ivec2& res,
                                    const float* __restrict__ cdf_x_cond_y,
                                    const float* __restrict__ cdf_y)
...
inline __device__ vec2 nerf_random_image_pos_training(default_rng_t& rng,
...
inline NGP_HOST_DEVICE uint32_t image_idx(uint32_t base_idx,
                                          uint32_t n_rays,
...
inline NGP_HOST_DEVICE LossAndGradient loss_and_gradient(const vec3& target,
                                                         ...
                                                         ELossType loss_type)
{
    ...
    default:
    case ELossType::L2: return l2_loss(target, prediction); break;
    }
}
