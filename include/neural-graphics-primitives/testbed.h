...
public:
    ...
    Testbed(ETestbedMode mode, const fs::path& data_path) : Testbed(mode)
    {
        load_training_data(data_path);
    }
    Testbed(ETestbedMode mode, const fs::path& data_path, const fs::path& network_config_path) :
        Testbed(mode, data_path)
    {
        reload_network_from_file(network_config_path);
    }
    Testbed(ETestbedMode mode, const fs::path& data_path, const nlohmann::json& network_config) :
        Testbed(mode, data_path)
    {
    ...
    public:
        ...
        uint32_t trace(const distance_fun_t& distance_function,
                       float zero_offset,
        ...
        RaysSdfSoa& rays_init()
        {
            return m_rays[0];
        }
        uint32_t n_rays_initialized() const
        {
            return m_n_rays_initialized;
        }
        void set_trace_shadow_rays(bool val)
        {
            m_trace_shadow_rays = val;
        }
        void set_shadow_sharpness(float val)
        {
            m_shadow_sharpness = val;
        }
    ...
    public:
        ...
        uint32_t traceSlices(const std::shared_ptr<NerfNetwork<network_precision_t>>& network,
                             const BoundingBox& render_aabb,
                             const mat3& render_aabb_to_local,
        ...
        uint32_t trace(const std::shared_ptr<NerfNetwork<network_precision_t>>& network,
                       const BoundingBox& render_aabb,
                       const mat3& render_aabb_to_local,
                       const BoundingBox& train_aabb,
                       const ivec2& resolution,
    ...
    public:
        ...
        void normal(uint32_t n_elements,
    ...
    struct LevelStats
    {
        float mean()
        {
            return count ? (x / (float) count) : 0.f;
        }
        float variance()
        {
        ...
        float fraczero()
        {
            return (float) numzero / float(count + numzero);
        }
        float fracquant()
        {
            return (float) numquant / float(count);
        }
    ...
    void render_nerf(cudaStream_t stream,
                     ...
                     const Foveation& foveation,
                     int visualized_dimension);
    void render_sdf(cudaStream_t stream,
                    const distance_fun_t& distance_function,
                    ...
                    const Foveation& foveation,
                    int visualized_dimension);
    void render_image(cudaStream_t stream,
                      const CudaRenderBufferView& render_buffer,
                      const vec2& focal_length,
                      const mat4x3& camera_matrix,
                      const vec2& screen_center,
                      const Foveation& foveation,
                      int visualized_dimension);
    void render_volume(cudaStream_t stream,
                       const CudaRenderBufferView& render_buffer,
                       const vec2& focal_length,
                       const mat4x3& camera_matrix,
                       const vec2& screen_center,
    ...
    void render_frame(cudaStream_t stream,
                      const mat4x3& camera_matrix0,
                      const mat4x3& camera_matrix1,
    ...
    void render_frame_main(CudaDevice& device,
                           const mat4x3& camera_matrix0,
                           const mat4x3& camera_matrix1,
                           const vec2& screen_center,
                           const vec2& relative_focal_length,
                           ...
                           const Foveation& foveation,
                           int visualized_dimension);
    void render_frame_epilogue(cudaStream_t stream,
                               const mat4x3& camera_matrix0,
                               const mat4x3& prev_camera_matrix,
                               const vec2& screen_center,
                               const vec2& relative_focal_length,
    ...
    void reload_network_from_json(
        ...
        const std::string& config_base_path =
            ""); // config_base_path is needed so that if the passed in json uses the 'parent' feature, we know where to look... be sure to use a filename, or if a directory, end with a trailing slash
    ...
    void reset_camera();
    bool keyboard_event();
    void generate_training_samples_sdf(vec3* positions,
                                       float* distances,
    ...
    GPUMemory<float> get_density_on_grid(ivec3 res3d,
                                         const BoundingBox& aabb,
                                         const mat3& render_aabb_to_local); // network version (nerf or sdf)
    GPUMemory<float> get_sdf_gt_on_grid(ivec3 res3d,
                                        const BoundingBox& aabb,
                                        const mat3& render_aabb_to_local); // sdf gt version (sdf only)
    GPUMemory<vec4> get_rgba_on_grid(ivec3 res3d,
                                     vec3 ray_dir,
                                     bool voxel_centers,
                                     float depth,
...
#ifdef NGP_PYTHON
    pybind11::dict compute_marching_cubes_mesh(ivec3 res3d      = ivec3(128),
                                               BoundingBox aabb = BoundingBox{vec3(0.0f), vec3(1.0f)},
                                               float thresh     = 2.5f);
    pybind11::array_t<float> render_to_cpu(int width,
                                           int height,
                                           int spp,
                                           bool linear,
                                           float start_t,
    ...
    void compute_and_save_marching_cubes_mesh(const fs::path& filename,
                                              ivec3 res3d      = ivec3(128),
                                              BoundingBox aabb = {},
                                              float thresh     = 2.5f,
                                              bool unwrap_it   = false);
    ivec3 compute_and_save_png_slices(const fs::path& filename,
                                      int res,
                                      BoundingBox aabb       = {},
                                      float thresh           = 2.5f,
    ...
    void blit_texture(const Foveation& foveation,
                      GLint rgba_texture,
    ...
    struct Nerf
    {
        struct Training
        {
            ...
            void set_camera_intrinsics(int frame_idx,
                                       float fx,
                                       float fy        = 0.0f,
                                       float cx        = -0.5f,
                                       float cy        = -0.5f,
            ...
            void set_camera_extrinsics_rolling_shutter(int frame_idx,
    ...
    struct Sdf
    {
        ...
        int octree_depth_target =
            0; // we duplicate this state so that you can waggle the slider without triggering it immediately
