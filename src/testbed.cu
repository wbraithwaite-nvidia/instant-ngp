...
json Testbed::load_network_config(const fs::path& network_config_path)
{
    ...
    if (network_config_path.empty() || !network_config_path.exists())
    {
        throw std::runtime_error{fmt::format(
            "Network {} '{}' does not exist.", is_snapshot ? "snapshot" : "config", network_config_path.str())};
    }
...
void Testbed::reset_camera()
{
    ...
    m_camera = transpose(mat3x4{1.0f, 0.0f, 0.0f, 0.5f, 0.0f, -1.0f, 0.0f, 0.5f, 0.0f, 0.0f, -1.0f, 0.5f});
...
void Testbed::compute_and_save_marching_cubes_mesh(const fs::path& filename,
                                                   ivec3 res3d,
                                                   BoundingBox aabb,
                                                   float thresh,
                                                   bool unwrap_it)
...
ivec3 Testbed::compute_and_save_png_slices(const fs::path& filename,
                                           int res,
                                           BoundingBox aabb,
                                           float thresh,
                                           float density_range,
...
void Testbed::dump_parameters_as_images(const T* params, const std::string& filename_base)
{
    ...
    std::vector<T> params_cpu_network_precision(layer_params +
                                                next_multiple(n_non_layer_params, non_layer_params_width));
    std::vector<float> params_cpu(params_cpu_network_precision.size(), 0.0f);
    CUDA_CHECK_THROW(
        cudaMemcpy(params_cpu_network_precision.data(), params, n_params * sizeof(T), cudaMemcpyDeviceToHost));
    ...
    for (auto size : m_network->layer_sizes())
    {
        save_exr(params_cpu.data() + offset,
                 ...
                 1,
                 1,
                 fmt::format("{}-layer-{}.exr", filename_base, layer_id).c_str());
    ...
    if (n_non_layer_params > 0)
    {
        ...
        save_exr(params_cpu.data() + offset,
                 non_layer_params_width,
                 n_non_layer_params / non_layer_params_width,
                 1,
                 1,
                 filename.c_str());
    }
...
void Testbed::imgui()
{
    ...
    if (ImGui::Begin("Camera path", 0, ImGuiWindowFlags_NoScrollbar))
    {
        ...
        {
            if (int read = m_camera_path.imgui(m_imgui.cam_path_path,
                                               m_render_ms.val(),
                                               m_camera,
                                               m_slice_plane_z,
                                               m_scale,
        ...
        if (!m_camera_path.keyframes.empty() && ImGui::CollapsingHeader("Export video", ImGuiTreeNodeFlags_DefaultOpen))
        {
            ...
            if (m_camera_path.rendering)
            {
                ...
                uint32_t progress = m_camera_path.render_frame_idx * m_camera_path.render_settings.spp +
                                    m_views.front().render_buffer->spp();
                ...
                ImGui::Text("%s",
                            fmt::format("Frame {}/{}, Elapsed: {}, Remaining: {}",
                                        m_camera_path.render_frame_idx + 1,
                                        m_camera_path.render_settings.n_frames(),
                                        tlog::durationToString(std::chrono::steady_clock::now() -
                                                               m_camera_path.render_start_time),
    ...
    if (train_extra_dims && m_nerf.training.n_images_for_training > 0)
    {
        if (ImGui::Begin("Latent space 2D embedding"))
        {
            ...
            for (uint32_t i = 0; i < n; ++i)
            {
                ...
                ImColor col(sinf(theta) * 0.4f + 0.5f,
                            sinf(theta + PI() * 2.0f / 3.0f) * 0.4f + 0.5f,
                            sinf(theta + PI() * 4.0f / 3.0f) * 0.4f + 0.5f);
    ...
    ImGui::Text("Frame: %.2f ms (%.1f FPS); Mem: %s",
                m_frame_ms.ema_val(),
                1000.0f / m_frame_ms.ema_val(),
                bytes_to_string(n_bytes).c_str());
    ...
    if (ImGui::CollapsingHeader("Training", m_training_data_available ? ImGuiTreeNodeFlags_DefaultOpen : 0))
    {
        ...
        if (m_testbed_mode == ETestbedMode::Nerf)
        {
            ...
            if (extrinsics_have_been_optimized)
            {
                ...
                ImGui::InputText(
                    "File##Extrinsics file path", m_imgui.extrinsics_path, sizeof(m_imgui.extrinsics_path));
            }
        }

        ImGui::PushItemWidth(ImGui::GetWindowWidth() * 0.3f);
        ImGui::SliderInt(
            "Batch size", (int*) &m_training_batch_size, 1 << 12, 1 << 22, "%d", ImGuiSliderFlags_Logarithmic);
        ImGui::SameLine();
        ...
        if (m_testbed_mode == ETestbedMode::Nerf)
        {
            ImGui::Text("Rays/batch: %d, Samples/ray: %.2f, Batch size: %d/%d",
        ...
        if (m_testbed_mode == ETestbedMode::Nerf && ImGui::TreeNode("NeRF training options"))
        {
            ...
            ImGui::SliderFloat("Extrinsic L2 reg.",
                               &m_nerf.training.extrinsic_l2_reg,
                               1e-8f,
                               0.1f,
                               "%.6f",
                               ImGuiSliderFlags_Logarithmic | ImGuiSliderFlags_NoRoundToFormat);
            ImGui::SliderFloat("Intrinsic L2 reg.",
                               &m_nerf.training.intrinsic_l2_reg,
                               1e-8f,
                               0.1f,
                               "%.6f",
                               ImGuiSliderFlags_Logarithmic | ImGuiSliderFlags_NoRoundToFormat);
            ImGui::SliderFloat("Exposure L2 reg.",
                               &m_nerf.training.exposure_l2_reg,
                               1e-8f,
                               0.1f,
                               "%.6f",
                               ImGuiSliderFlags_Logarithmic | ImGuiSliderFlags_NoRoundToFormat);
        ...
        if (m_testbed_mode == ETestbedMode::Sdf && ImGui::TreeNode("SDF training options"))
        {
            ...
            accum_reset |= ImGui::SliderFloat("Surface offset scale",
        ...
        if (m_testbed_mode == ETestbedMode::Volume && ImGui::TreeNode("Volume training options"))
        {
            ...
            accum_reset |= ImGui::SliderFloat("Distance scale",
                                              &m_volume.inv_distance_scale,
                                              1.f,
                                              100.f,
                                              "%.3g",
                                              ImGuiSliderFlags_Logarithmic | ImGuiSliderFlags_NoRoundToFormat);
    ...
    if (ImGui::CollapsingHeader("Rendering", ImGuiTreeNodeFlags_DefaultOpen))
    {
        ...
        else
        {
            ...
            else if (ImGui::TreeNodeEx("VR/AR settings", ImGuiTreeNodeFlags_DefaultOpen))
            {
                ...
                {
                    if (ImGui::Combo(
                            "Environment", &blend_mode_idx, m_hmd->supported_environment_blend_modes_imgui_string()))
                    {
                ...
                if (m_foveated_rendering)
                {
                    ...
                    if (ImGui::TreeNodeEx("Foveated rendering settings"))
                    {
                        ...
                        {
                            accum_reset |=
                                ImGui::SliderFloat("Maximum scaling",
                        ...
                        accum_reset |=
                            ImGui::SliderFloat("Fovea diameter", &m_foveated_rendering_full_res_diameter, 0.1f, 0.9f) &&
                            !m_dlss;
                    }
                }
        ...
        {
            if (ImGui::SliderFloat("Crop size",
                                   &render_diam,
                                   0.1f,
                                   max_diam,
                                   "%.3f",
        ...
        if (ImGui::TreeNode(transform_section_name.c_str()))
        {
            ...
            if (ImGui::RadioButton("Translate world",
                                   m_camera_path.m_gizmo_op == ImGuizmo::TRANSLATE && m_edit_world_transform))
            {
            ...
            ImGui::SameLine();
            if (ImGui::RadioButton("Rotate world",
                                   m_camera_path.m_gizmo_op == ImGuizmo::ROTATE && m_edit_world_transform))
            {
            ...
            if (m_testbed_mode == ETestbedMode::Nerf)
            {
                if (ImGui::RadioButton("Translate crop box",
                                       m_camera_path.m_gizmo_op == ImGuizmo::TRANSLATE && !m_edit_world_transform))
                {
                    m_camera_path.m_gizmo_op = ImGuizmo::TRANSLATE;
                ...
                ImGui::SameLine();
                if (ImGui::RadioButton("Rotate crop box",
                                       m_camera_path.m_gizmo_op == ImGuizmo::ROTATE && !m_edit_world_transform))
                {
                    m_camera_path.m_gizmo_op = ImGuizmo::ROTATE;
                    m_edit_world_transform   = false;
                ...
                accum_reset |= ImGui::SliderFloat(
                    "Min x", ((float*) &m_render_aabb.min) + 0, m_aabb.min.x, m_render_aabb.max.x, "%.3f");
                accum_reset |= ImGui::SliderFloat(
                    "Min y", ((float*) &m_render_aabb.min) + 1, m_aabb.min.y, m_render_aabb.max.y, "%.3f");
                accum_reset |= ImGui::SliderFloat(
                    "Min z", ((float*) &m_render_aabb.min) + 2, m_aabb.min.z, m_render_aabb.max.z, "%.3f");
                ImGui::Separator();
                accum_reset |= ImGui::SliderFloat(
                    "Max x", ((float*) &m_render_aabb.max) + 0, m_render_aabb.min.x, m_aabb.max.x, "%.3f");
                accum_reset |= ImGui::SliderFloat(
                    "Max y", ((float*) &m_render_aabb.max) + 1, m_render_aabb.min.y, m_aabb.max.y, "%.3f");
                accum_reset |= ImGui::SliderFloat(
                    "Max z", ((float*) &m_render_aabb.max) + 2, m_render_aabb.min.z, m_aabb.max.z, "%.3f");
                ImGui::Separator();
        ...
        {
            ImGui::SliderInt(
                "Max spp", &m_max_spp, 0, 1024, "%d", ImGuiSliderFlags_Logarithmic | ImGuiSliderFlags_NoRoundToFormat);
            accum_reset |=
                ImGui::Checkbox("Render transparency as checkerboard", &m_render_transparency_as_checkerboard);
        ...
        if (m_testbed_mode == ETestbedMode::Nerf && ImGui::TreeNode("NeRF rendering options"))
        {
            ...
            {
                accum_reset |= ImGui::SliderInt("Rendering extra dims from training view",
            ...
            accum_reset |=
                ImGui::Combo("Groundtruth render mode", (int*) &m_ground_truth_render_mode, GroundTruthRenderModeStr);
            accum_reset |= ImGui::SliderFloat(
                "Groundtruth alpha", &m_ground_truth_alpha, 0.0f, 1.0f, "%.02f", ImGuiSliderFlags_AlwaysClamp);
            ...
            accum_reset |= ImGui::SliderFloat("Min transmittance",
        ...
        if (m_testbed_mode == ETestbedMode::Sdf && ImGui::TreeNode("SDF rendering options"))
        {
            ...
            accum_reset |= ImGui::SliderFloat(
                "Normals epsilon", &m_sdf.fd_normals_epsilon, 0.00001f, 0.1f, "%.6g", ImGuiSliderFlags_Logarithmic);
            accum_reset |= ImGui::SliderFloat(
                "Maximum distance", &m_sdf.maximum_distance, 0.00001f, 0.1f, "%.6g", ImGuiSliderFlags_Logarithmic);
            accum_reset |= ImGui::SliderFloat(
                "Shadow sharpness", &m_sdf.shadow_sharpness, 0.1f, 2048.0f, "%.6g", ImGuiSliderFlags_Logarithmic);
        ...
        if (ImGui::TreeNode("Debug visualization"))
        {
            ...
            if (ImGui::SliderInt(
                    "Visualized dimension", &m_visualized_dimension, -1, (int) network_width(m_visualized_layer) - 1))
            {
            ...
            if (ImGui::SliderInt("Visualized layer",
                                 &m_visualized_layer,
                                 0,
                                 std::max(0, (int) network_num_forward_activations() - 1)))
            {
            ...
            if (m_testbed_mode == ETestbedMode::Nerf)
            {
                ...
                if (ImGui::SliderInt(
                        "Training view", &m_nerf.training.view, 0, (int) m_nerf.training.dataset.n_images - 1))
                {
                    set_camera_to_training_view(m_nerf.training.view);
                    accum_reset = true;
                }
                ...
                if (m_nerf.training.optimize_exposure)
                {
                    ...
                    ImGui::PlotLines("Training view exposures",
                                     exposures.data(),
                                     exposures.size(),
                                     0,
                                     nullptr,
    ...
    if (ImGui::CollapsingHeader("Camera", ImGuiTreeNodeFlags_DefaultOpen))
    {
        ...
        ImGui::PushItemWidth(ImGui::GetWindowWidth() * 0.3f);
        if (ImGui::SliderFloat("Aperture size",
                               &m_aperture_size,
                               0.0f,
                               1.0f,
                               "%.3f",
        ...
        if (ImGui::TreeNode("Advanced camera settings"))
        {
            ...
            accum_reset |=
                ImGui::SliderFloat("Slice / focus depth", &m_slice_plane_z, -m_bounding_radius, m_bounding_radius);
            accum_reset |= ImGui::SliderFloat("Render near distance",
    ...
    if (m_testbed_mode == ETestbedMode::Nerf || m_testbed_mode == ETestbedMode::Sdf)
    {
        if (ImGui::CollapsingHeader("Export mesh / volume / slices"))
        {
            ...
            bool disable_marching_cubes =
                m_testbed_mode == ETestbedMode::Sdf && (m_sdf.uses_takikawa_encoding || m_sdf.use_triangle_octree);
            if (disable_marching_cubes)
            {
                ImGui::BeginDisabled();
            }
            ...
            if (imgui_colored_button("Save density PNG", -0.7f))
            {
                compute_and_save_png_slices(
                    m_data_path, m_mesh.res, {}, m_mesh.thresh, density_range, flip_y_and_z_axes);
            }
            ...
            if (m_testbed_mode == ETestbedMode::Nerf)
            {
                ...
                if (imgui_colored_button("Save RGBA PNG sequence", 0.2f))
                {
                    ...
                    auto dir =
                        m_data_path.is_directory() || m_data_path.empty()
...
void Testbed::visualize_nerf_cameras(ImDrawList* list, const mat4& world2proj)
{
    for (int i = 0; i < m_nerf.training.n_images_for_training; ++i)
    {
        ...
        add_debug_line(list,
                       world2proj,
                       current_xform[3],
                       current_xform[3] + current_xform[2] * m_nerf.training.near_distance,
                       0x20ffffff);
    }
}
...
void Testbed::draw_visualizations(ImDrawList* list, const mat4x3& camera_matrix)
{
    ...
    if (m_edit_render_aabb)
    {
        ...
        if (ImGuizmo::Manipulate((const float*) &world2view_guizmo,
                                 (const float*) &view2proj_guizmo,
                                 m_camera_path.m_gizmo_op,
                                 ImGuizmo::LOCAL,
                                 (float*) &matrix,
                                 NULL,
                                 NULL))
        ...
        if (m_single_view)
        {
                ...
                m_visualized_layer < network_num_forward_activations() - 1)
            {
                set_visualized_layer(
                    std::max(0, std::min((int) network_num_forward_activations() - 1, m_visualized_layer + 1)));
                set_visualized_dim(0);
            }
            else
            {
                set_visualized_dim(
                    std::max(-1, std::min((int) network_width(m_visualized_layer) - 1, m_visualized_dimension + 1)));
            }
        }
        else
        {
...
void Testbed::handle_user_input()
{
    ...
    if (m_testbed_mode == ETestbedMode::Nerf && (m_render_ground_truth || m_nerf.training.render_error_overlay))
    {
        ...
        { // snap camera to ground truth view on mouse up
            set_camera_to_training_view(m_nerf.training.view);
...
void Testbed::begin_vr_frame_and_handle_vr_input()
{
    ...
    if (n_views > 0)
    {
        ...
        {
            ivec2 view_resolution = {views[i].view.subImage.imageRect.extent.width,
                                     views[i].view.subImage.imageRect.extent.height};
            ...
            m_views[i].relative_focal_length[(m_fov_axis + 1) % 2] *=
                (float) view_resolution[(m_fov_axis + 1) % 2] / (float) view_resolution[m_fov_axis];
            m_views[i].render_buffer->set_hidden_area_mask(m_vr_use_hidden_area_mask ? views[i].hidden_area_mask
                                                                                     : nullptr);
        ...
        if (!hands[0].pressing)
        {
            vec3 translate_vec = vec3{hands[0].thumbstick.x, 0.0f, hands[0].thumbstick.y} * m_camera_velocity *
                                 m_frame_ms.val() / 1000.0f;
            if (translate_vec != vec3(0.0f))
            {
        ...
        if (!hands[1].pressing)
        {
            ...
            auto rot          = rotation_from_angles(
                           {-2.0f * PI() * sensitivity * hands[1].thumbstick.x * m_frame_ms.val() / 1000.0f, 0.0f}) *
                       mat3(m_camera);
            m_camera = mat4x3(rot[0], rot[1], rot[2], m_camera[3]);
                           ...
                           const ivec2& resolution)
{
    ...
    auto bind_warp = [&](const FoveationPiecewiseQuadratic& warp, const std::string& uniform_name) {
        ...
        glUniform1f(glGetUniformLocation(m_blit_program, (uniform_name + ".inv_switch_left").c_str()),
                    warp.inv_switch_left);
        glUniform1f(glGetUniformLocation(m_blit_program, (uniform_name + ".inv_switch_right").c_str()),
                    warp.inv_switch_right);
    };
...
void Testbed::draw_gui()
{
    ...
    if (m_views.size() == 1)
    {
        ...
        {
            list->AddCallback(
                [](const ImDrawList*, const ImDrawCmd* cmd) { (*(decltype(draw_mesh)*) cmd->UserCallbackData)(); },
                &draw_mesh);
            list->AddCallback(ImDrawCallback_ResetRenderState, nullptr);
        }
...
__global__ void to_8bit_color_kernel(ivec2 resolution,
...
void Testbed::prepare_next_camera_path_frame()
{
    ...
    if (m_views.front().render_buffer->spp() == m_camera_path.render_settings.spp)
    {
        ...
        to_8bit_color_kernel<<<blocks, threads>>>(res,
                                                  EColorSpace::SRGB, // the GUI always renders in SRGB
                                                  m_views.front().render_buffer->surface(),
                                                  image_data.data());

        m_render_futures.emplace_back(m_thread_pool.enqueue_task(
            [image_data = std::move(image_data), frame_idx = m_camera_path.render_frame_idx++, res, tmp_dir] {
                std::vector<uint8_t> cpu_image_data(image_data.size());
                CUDA_CHECK_THROW(
                    cudaMemcpy(cpu_image_data.data(), image_data.data(), image_data.bytes(), cudaMemcpyDeviceToHost));
        ...
        if (m_camera_path.render_frame_idx == m_camera_path.render_settings.n_frames())
        {
            ...
            tlog::success() << "Finished rendering '.jpg' video frames to '" << tmp_dir
                            << "'. Assembling them into a video next.";
            ...
            auto ffmpeg_command = fmt::format(
                "{} -loglevel error -y -framerate {} -i tmp/%06d.jpg -c:v libx264 -preset slow -crf {} -pix_fmt "
                "yuv420p \"{}\"",
                ffmpeg.str(),
...
void Testbed::train_and_render(bool skip_rendering)
{
#if 1
    //set_n_views(1);
    /*// this seems silly to to here.
    // because of this we need to setup GL beforehand!
    if (m_rgba_render_textures.size() == 0)
        m_rgba_render_textures = {std::make_shared<ngp::GLTexture>()};
    if (m_depth_render_textures.size() == 0)
        m_depth_render_textures = {std::make_shared<ngp::GLTexture>()};
    if (m_views.size() == 0)
    {
        m_views.emplace_back(ngp::Testbed::View{std::make_shared<ngp::CudaRenderBuffer>(
            m_rgba_render_textures.front(), mImpl->m_depth_render_textures.front())});
        m_views.front().full_resolution = ivec2(256, 256);
        m_views.front().render_buffer->resize(mImpl->m_views.front().full_resolution);
    }*/
#endif
    ...
    if (!m_camera_path.rendering && frobenius_norm(smoothed_camera_backup - m_smoothed_camera) > 0.001f)
        reset_accumulation(true);
    ...
    if (m_hmd && m_hmd->is_visible())
    {
        ...
        if (m_single_view)
        {
            ...
            view.camera1 = m_camera_path.rendering ? camera_log_lerp(m_smoothed_camera,
        ...
        for (auto&& view : m_views)
        {
            ...
            if (m_foveated_rendering)
            {
                if (m_dynamic_foveated_rendering)
                {
                    ...
                    resolution_scale = clamp(resolution_scale * foveation_begin_factor,
                                             vec2(1.0f / m_foveated_rendering_max_scaling),
                                             vec2(1.0f));
                ...
                {
                    view.foveation = {vec2(1.0f / m_foveated_rendering_scaling),
                                      vec2(1.0f) - view.screen_center,
                                      vec2(m_foveated_rendering_full_res_diameter * 0.5f)};
                }
            }
...

    {
        ...
        for (size_t i = 0; i < m_views.size(); ++i)
        {
            ...
            const bool to_srgb = false; // wil: ingp was true
    ...
    if (m_picture_in_picture_res > 0)
    {
        ...
        if (m_pip_render_buffer->spp() < 8)
        {
            ...
            m_pip_render_texture->blit_to_surface();
        }
    }
#endif
...
void Testbed::set_n_views(size_t n_views)
{
    ...
    while (m_views.size() < n_views)
    {
        ...
        m_views.emplace_back(
            View{std::make_shared<CudaRenderBuffer>(m_rgba_render_textures[idx], m_depth_render_textures[idx])});
    }
#else
    while (m_views.size() < n_views)
    {
        m_views.emplace_back(View{std::make_shared<CudaRenderBuffer>()});
    }
#endif
};
#endif //NGP_GUI
...
void Testbed::init_window(int resw, int resh, bool hidden, bool second_window)
{
    ...
    if (gl_version_major < 3 || (gl_version_major == 3 && gl_version_minor < 1))
    {
        throw std::runtime_error{
            fmt::format("Unsupported OpenGL version {}.{}. instant-ngp requires at least OpenGL 3.1",
                        gl_version_major,
                        gl_version_minor)};
    }
    ...
    m_views.clear();
    m_views.emplace_back(
        View{std::make_shared<CudaRenderBuffer>(m_rgba_render_textures.front(), m_depth_render_textures.front())});
...
void Testbed::init_vr()
{
    ...
    try
    {
        ...
        m_hmd = std::make_unique<OpenXRHMD>(
            xDisplay, visualInfo->visualid, glxFBConfig, glXGetCurrentDrawable(), glxContext);
    ...
    catch (const std::runtime_error& e)
    {
        ...
        {
            throw std::runtime_error{
                "Could not initialize VR. Ensure that SteamVR, OculusVR, or any other OpenXR-compatible runtime is "
                "running. Also set it as the active OpenXR runtime."};
        }
        else
        {
...
void Testbed::reset_network(bool clear_density_grid)
{
    ...
    if (to_lower(encoding_config.value("otype", "OneBlob")).find("grid") != std::string::npos)
    {
        ...
        {
            m_per_level_scale = std::exp(std::log(desired_resolution * (float) m_nerf.training.dataset.aabb_scale /
        ...
        tlog::info() << "GridEncoding: "
                     << " Nmin=" << m_base_grid_resolution << " b=" << m_per_level_scale
    ...
    if (m_testbed_mode == ETestbedMode::Nerf)
    {
        ...
        // Create distortion map model
        {
            json& distortion_map_optimizer_config =
                config.contains("distortion_map") && config["distortion_map"].contains("optimizer")
    ...
    {
        uint32_t alignment =
            network_config.contains("otype") && (equals_case_insensitive(network_config["otype"], "FullyFusedMLP") ||
    ...
    // Create envmap model
    {
        json& envmap_loss_config =
            config.contains("envmap") && config["envmap"].contains("loss") ? config["envmap"]["loss"] : loss_config;
        json& envmap_optimizer_config = config.contains("envmap") && config["envmap"].contains("optimizer")
...
Testbed::Testbed(ETestbedMode mode)
{
    ...
    tlog::success() << fmt::format("Initialized CUDA {}. Active GPU is #{}: {} [{}]",
...
vec2 Testbed::calc_focal_length(const ivec2& resolution,
...
__global__ void dlss_prep_kernel(ivec2 resolution,
                                 ...
                                 Lens lens)
{
    ...
    vec2 mvec         = motion_vector(sample_index,
                                      {(int) x, (int) y},
                              resolution,
                              focal_length,
                              camera,
...
__global__ void spherical_checkerboard_kernel(ivec2 resolution,
                                              vec2 focal_length,
                                              mat4x3 camera,
                                              vec2 screen_center,
                                              vec3 parallax_shift,
...
__global__ void vr_overlay_hands_kernel(ivec2 resolution,
                                        vec2 focal_length,
                                        mat4x3 camera,
                                        vec2 screen_center,
                                        vec3 parallax_shift,
                                ...
                                int visualized_dimension)
{
    ...
    vec2 focal_length =
        calc_focal_length(device.render_buffer_view().resolution, relative_focal_length, m_fov_axis, m_zoom);
    vec2 screen_center = render_screen_center(orig_screen_center);
    ...
    case ETestbedMode::Sdf: {
        if (m_render_ground_truth && m_sdf.groundtruth_mode == ESDFGroundTruthMode::SDFBricks)
        {
            ...
            {
                tlog::info() << "Building voxel brick positions for " << m_sdf.triangle_octree->n_dual_nodes()
                             << " dual nodes.";
                m_sdf.brick_res             = 5;
                ...
                m_sdf.triangle_bvh->signed_distance_gpu(
                    positions.size(),
                    EMeshSdfMode::
                        Watertight, //m_sdf.mesh_sdf_mode, // watertight seems to be the best method for 'one off' SDF signing
                    positions_gpu.data(),
    ...
    case ETestbedMode::Volume:
        render_volume(
            device.stream(), device.render_buffer_view(), focal_length, camera_matrix0, screen_center, foveation);
        break;
    default:
...
void Testbed::render_frame_epilogue(cudaStream_t stream,
                                    const mat4x3& camera_matrix0,
                                    const mat4x3& prev_camera_matrix,
                                    const vec2& orig_screen_center,
                                    const vec2& relative_focal_length,
                                    ...
                                    bool to_srgb)
{
    ...
    Lens lens =
        (m_testbed_mode == ETestbedMode::Nerf && m_nerf.render_with_lens_distortion) ? m_nerf.render_lens : Lens{};
    ...
    if (render_buffer.dlss())
    {
        ...
        dlss_prep_kernel<<<blocks, threads, 0, stream>>>(res,
                                                         render_buffer.spp(),
                                                         focal_length,
                                                         screen_center,
                                                         m_parallax_shift,
    ...
    if (m_render_transparency_as_checkerboard)
    {
        ...
        const dim3 threads = {16, 8, 1};
        const dim3 blocks  = {div_round_up((uint32_t) res.x, threads.x), div_round_up((uint32_t) res.y, threads.y), 1};
        spherical_checkerboard_kernel<<<blocks, threads, 0, stream>>>(res,
                                                                      focal_length,
    ...
    render_buffer.accumulate(m_exposure, stream);
    render_buffer.tonemap(
        m_exposure, m_background_color, output_color_space, m_ndc_znear, m_ndc_zfar, m_snap_to_pixel_centers, stream);
    ...
    if (m_testbed_mode == ETestbedMode::Nerf)
    {
        ...
        if (m_render_ground_truth)
        {
            ...
            {
                render_buffer.overlay_depth(m_ground_truth_alpha,
                                            metadata.depth,
        ...
        if (m_nerf.training.render_error_overlay)
        {
            ...
            reduce_sum(
                ...
                stream);
            auto const& metadata = m_nerf.training.dataset.metadata[m_nerf.training.view];
            render_buffer.overlay_false_color(metadata.resolution,
                                              to_srgb,
                                              m_fov_axis,
                                              stream,
                                              err_data,
...
float Testbed::get_depth_from_renderbuffer(const CudaRenderBuffer& render_buffer, const vec2& uv)
{
    ...
    CUDA_CHECK_THROW(cudaMemcpy(&depth,
                                render_buffer.depth_buffer() + depth_pixel.x + depth_pixel.y * res.x,
                                sizeof(float),
...
vec3 Testbed::get_3d_pos_from_pixel(const CudaRenderBuffer& render_buffer, const vec2& pixel)
{
    ...
    auto ray    = pixel_to_ray_pinhole(0,
                                    ivec2(pixel),
                                    m_window_res,
...
void Testbed::gather_histograms()
{
    ...
    if (hg_enc && m_trainer->params())
    {
        ...
        {
            m_level_stats[l] =
                compute_level_stats(grid.data() + hg_enc->level_params_offset(l), hg_enc->level_n_params(l));
        }
...
void Testbed::save_snapshot(const fs::path& path, bool include_optimizer_state, bool compress)
{
    ...
    if (m_testbed_mode == ETestbedMode::Nerf)
    {
        ...
        parallel_for_gpu(
            ...
            [density_grid = m_nerf.density_grid.data(), density_grid_fp16 = density_grid_fp16.data()] __device__(
                size_t i) { density_grid_fp16[i] = (__half) density_grid[i]; });
...
void Testbed::load_snapshot(nlohmann::json config)
{
    ...
    if (m_testbed_mode == ETestbedMode::Nerf)
    {
        ...
        else
        {
            ...
            {
                m_nerf.training.dataset.n_extra_learnable_dims = snapshot["nerf"]["dataset"].value(
        ...
        parallel_for_gpu(
            density_grid_fp16.size(),
            [density_grid = m_nerf.density_grid.data(), density_grid_fp16 = density_grid_fp16.data()] __device__(
                size_t i) { density_grid[i] = (float) density_grid_fp16[i]; });
    ...
    if (m_testbed_mode == ETestbedMode::Nerf)
    {
        ...
        if (snapshot["nerf"].contains("dataset") && m_nerf.training.dataset.is_same(snapshot["nerf"]["dataset"]))
        {
            if (snapshot["nerf"].contains("cam_pos_offset"))
                m_nerf.training.cam_pos_offset =
                    snapshot["nerf"].at("cam_pos_offset").get<std::vector<AdamOptimizer<vec3>>>();
            if (snapshot["nerf"].contains("cam_rot_offset"))
                m_nerf.training.cam_rot_offset =
                    snapshot["nerf"].at("cam_rot_offset").get<std::vector<RotationAdamOptimizer>>();
            if (snapshot["nerf"].contains("extra_dims_opt"))
                m_nerf.training.extra_dims_opt =
                    snapshot["nerf"].at("extra_dims_opt").get<std::vector<VarAdamOptimizer>>();
...
void Testbed::sync_device(CudaRenderBuffer& render_buffer, Testbed::CudaDevice& device)
{
    ...
    {
        CUDA_CHECK_THROW(cudaMemcpyPeerAsync(device.data().density_grid_bitfield.data(),
                                             device.id(),
    ...
    if (render_buffer.hidden_area_mask())
    {
        ...
        CUDA_CHECK_THROW(cudaMemcpyPeerAsync(ham->data(),
                                             device.id(),
                                             render_buffer.hidden_area_mask()->data(),
                                             active_device,
                                             ham->bytes(),
                                             device.stream()));
...
ScopeGuard Testbed::use_device(cudaStream_t stream, CudaRenderBuffer& render_buffer, Testbed::CudaDevice& device)
{
    ...
    return ScopeGuard{make_copyable_function(
        [&render_buffer, &device, guard = std::move(guard), alloc = std::move(alloc), active_device, stream]() {
            // Copy device's render buffer's data onto the original render buffer
            CUDA_CHECK_THROW(cudaMemcpyPeerAsync(render_buffer.frame_buffer(),
                                                 active_device,
                                                 device.render_buffer_view().frame_buffer,
                                                 device.id(),
                                                 product(render_buffer.in_resolution()) * sizeof(vec4),
                                                 device.stream()));
            CUDA_CHECK_THROW(cudaMemcpyPeerAsync(render_buffer.depth_buffer(),
                                                 active_device,
